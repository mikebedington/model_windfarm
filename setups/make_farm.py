...
#######################################################################################################################################################################
def make_farm(exp_tag, col_ll, start_dt, end_dt, farm_params, fabm_template='fabm_template.yaml', ext_force='vel_avg', make_restart=None, starting_carbon = 10000):
    ...
    """
    area_wind_farm = farm_params['area_wind_farm'] # in km**2
    no_turbines = farm_params['no_turbines']

    deps = np.arange(0,col_depth) # deps by metre
    ## Parameters for the surface part of the floating wind farm
    surface_depth = farm_params['surface_depth']
    surface_diameter = farm_params['surface_diameter']

    ## Parameters for the seabed part of the floating wind farm
    bed_depth = farm_params['bed_depth']
    bed_diameter = farm_params['bed_diameter']

    ## Parameters for mooring cable
    cable_diameter = farm_params['cable_diameter']
    cable_extension = farm_params['cable_extension']
    ...
    mid_lays[np.logical_or(bed_lays, surf_lays)] = False
    ...
    os.system(f'cp {fabm_template} ./{exp_tag}/fabm.yaml')

    if make_restart is not None:
        # Ersem setup
        ...
        os.system(f'cp {make_restart} ./{exp_tag}/restart.nc')
        donor_file = f'./{exp_tag}/restart.nc'
